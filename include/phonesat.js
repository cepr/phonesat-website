...
// Add local Blue Marble layer
L.tileLayer(
		'bluemarble/{z}/{x}/{y}.jpg',
		{
			attribution: '&copy; <a href="http://visibleearth.nasa.gov">NASA Blue Marble</a>',
			maxZoom: 5,
			tms: true
		}
).addTo(map);
...
L.tileLayer(
		'https://api.mapbox.com/styles/v1/mapbox/satellite-streets-v9/tiles/256/{z}/{x}/{y}?access_token={accessToken}',
		{
			...
			minZoom: 6,
			maxZoom: 18,
			accessToken: 'pk.eyJ1IjoiY3ByaXNjYWwiLCJhIjoiY2lrMXloOW1oMDNldXU2bTB4ZHVscXJ1NCJ9.aIHVzLmiEz-O1kOkJBzxEg'
		}
).addTo(map);

// Clicking on the map creates or moves the ground station
map.on('click', function (e) {
	...
	if (ground_station_marker) {
		// Move the current marker
		ground_station_marker.setLatLng(e.latlng);
	} else {
		// No group station defined yet
		// Show the tracking windows
		// Experimental: put the tracking window inside the marker popup?
		//ground_station_marker.bindPopup(polar_window);
		//ground_station_marker.openPopup();
	}
